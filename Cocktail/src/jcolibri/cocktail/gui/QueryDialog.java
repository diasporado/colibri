...
import jcolibri.cocktail.RecipeDescription;
import jcolibri.cocktail.CocktailRecipeRecommender;
import jcolibri.cocktail.RecipeDescription.AccommodationTypes;
import jcolibri.cocktail.RecipeDescription.Seasons;
...
public class QueryDialog extends JDialog {
	...
	private void configureFrame()
	{
		...
		image = new JLabel();
		image.setIcon(new ImageIcon(FileIO.findFile("jcolibri/cocktail/gui/step1.png")));
...
//		Lay out the panel.
		...
		exit.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				try {
					CocktailRecipeRecommender.getInstance().postCycle();
				} catch (Exception ex) {
					org.apache.commons.logging.LogFactory.getLog(CocktailRecipeRecommender.class).error(ex);
				}
				System.exit(-1);
			}
		});
